 ...
 if (isset($_POST["workerID"])) 
{
   ...
   $query1 = "SELECT COUNT(*) AS `count1` FROM `MISSIONS`  WHERE `WORKERID` = '".$workerid."'  ";
   $result1 = mysql_query($query1);
   
   $query2 = "SELECT * FROM `MISSIONS` WHERE `STATUS` = 'In Work' AND `WORKERID` = '".$workerid."' ";
   $result2 = mysql_query($query2);
   $count2= mysql_num_rows($result2);
   
   $query3 = "SELECT * FROM `MISSIONS` WHERE `STATUS` = 'Late' AND `WORKERID` = '".$workerid."'";
   $result3 = mysql_query($query3);
   $count3= mysql_num_rows($result3);
   
   $query4 = "SELECT * FROM `MISSIONS` WHERE `STATUS` = 'Done' AND `WORKERID` = '".$workerid."'";
